...
using namespace std; // add for mac
...
DWORD CCrc32Dynamic::FileCrc32Streams(LPCTSTR szFilename, DWORD &dwCrc32) const
{
	...
	try
	{
		...
		// Open the file
		file.open(szFilename, ios::in | ios::binary);  // removed , filebuf::sh_read
		if(!file.is_open())
			dwErrorCode = file.fail();
		else
		{
			char buffer[MAX_BUFFER_SIZE];
			long nLoop, nCount; // changed from int to long based on warning
