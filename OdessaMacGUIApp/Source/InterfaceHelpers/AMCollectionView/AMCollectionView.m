...
- (id)initWithFrame:(NSRect)frame
{
	...
	if (self) {
		...
		[self setItemsForObjects:[[NSMutableDictionary alloc] init]];
	}
	return self;
}
...
{
	NSColor *result = selectedRowColor;
	if (!result) {
...
{
	NSColor *result = secondarySelectedRowColor;
	if (!result) {
...
- (void)setArchivedItemPrototype:(NSData *)value
{
	if (am_archivedItemPrototype != value) {
		am_archivedItemPrototype = value;
	}
}
...
//	NSLog(@"drawRect:%f, %f, %f, %f", rect.origin.x, rect.origin.y, rect.size.width, rect.size.height);
    ...
    // REMED out because this was drawing a red line for some unknown reason!
