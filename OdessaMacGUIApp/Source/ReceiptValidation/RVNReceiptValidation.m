...
inline static void RVNCheckBundleIDAndVersion(void)
{
    ...
    if (![bundleID isEqualToString:kRVNBundleID]) {
        [NSException raise:@"MacAppStore Receipt Validation Error 1" format:@"Failed to check bundle ID.", nil];
    }
    ...
    if (![bundleVersion isEqualToString:kRVNBundleVersion]) {
        [NSException raise:@"MacAppStore Receipt Validation Error 2" format:@"Failed to check bundle Version.", nil];
    }
}
...
inline static void RVNCheckBundleSignature(void)
{
    ...
    if (status != errSecSuccess) {
        [NSException raise:@"MacAppStore Receipt Validation Error 3" format:@"Failed to validate bundle signature: Create a static code", nil];
    }
    ...
    if (status != errSecSuccess) {
        ...
        [NSException raise:@"MacAppStore Receipt Validation Error 4" format:@"Failed to validate bundle signature: Create a requirement", nil];
    }
    ...
    if (status != errSecSuccess) {
        ...
        [NSException raise:@"MacAppStore Receipt Validation Error 5" format:@"Failed to validate bundle signature: Check the static code validity", nil];
    }
...
inline static NSData *RVNGetReceiptData(void)
{
    ...
    else
    {
        ...
        receiptURL = [NSURL fileURLWithPath:receiptPath];
    }
    
    NSLog(@"path %@", receiptURL.path);
    
    NSData *receiptData = [NSData dataWithContentsOfURL:receiptURL];
    if (!receiptData) {
        [NSException raise:@"MacAppStore Receipt Validation Error 6" format:@"Failed to fetch the MacAppStore receipt.", nil];
    }
    return receiptData;
}
...
inline static NSData *RVNDecodeReceiptData(NSData *receiptData)
{
    ...
    @try {
        ...
        if (status) {
            [NSException raise:@"MacAppStore Receipt Validation Error 7" format:@"Failed to decode receipt data: Create a decoder", nil];
        }
        
        // Decrypt the message (1)
        status = CMSDecoderUpdateMessage(decoder, receiptData.bytes, receiptData.length);
        if (status) {
            [NSException raise:@"MacAppStore Receipt Validation Error 8" format:@"Failed to decode receipt data: Update message", nil];
        }
        
        // Decrypt the message (2)
        status = CMSDecoderFinalizeMessage(decoder);
        if (status) {
            [NSException raise:@"MacAppStore Receipt Validation Error 9" format:@"Failed to decode receipt data: Finalize message", nil];
        }
        ...
        if (status) {
            [NSException raise:@"MacAppStore Receipt Validation Error 10" format:@"Failed to decode receipt data: Get decrypted content", nil];
        }
        ...
        if (status) {
            [NSException raise:@"MacAppStore Receipt Validation Error 11" format:@"Failed to check receipt signature: Get singer count", nil];
        }
        if (numSigners == 0) {
            [NSException raise:@"MacAppStore Receipt Validation Error 12" format:@"Failed to check receipt signature: No signer found", nil];
        }
        ...
        if (status) {
            [NSException raise:@"MacAppStore Receipt Validation Error 13" format:@"Failed to check receipt signature: Get signer status", nil];
        }
        if (signerStatus != kCMSSignerValid) {
            [NSException raise:@"MacAppStore Receipt Validation Error 14" format:@"Failed to check receipt signature: No valid signer", nil];
        }
...
inline static NSNumber *RVNDecodeIntNumberFromASN1Data(SecAsn1CoderRef decoder, ASN1_Data srcData)
{
    ...
    if (status) {
        [NSException raise:@"MacAppStore Receipt Validation Error 15" format:@"Failed to get receipt information: Decode integer value", nil];
    }
...
inline static NSString *RVNDecodeUTF8StringFromASN1Data(SecAsn1CoderRef decoder, ASN1_Data srcData)
{
    ...
    if (status) {
        [NSException raise:@"MacAppStore Receipt Validation Error 16" format:@"Failed to get receipt information: Decode UTF-8 string", nil];
    }
...
inline static NSDate *RVNDecodeDateFromASN1Data(SecAsn1CoderRef decoder, ASN1_Data srcData)
{
    ...
    if (status) {
        [NSException raise:@"MacAppStore Receipt Validation Error 17" format:@"Failed to get receipt information: Decode date (IA5 string)", nil];
    }
...
inline static NSDictionary *RVNGetReceiptPayload(NSData *payloadData)
{
    ...
    @try {
        ...
        if (status) {
            [NSException raise:@"MacAppStore Receipt Validation Error 18" format:@"Failed to get receipt information: Create ASN.1 decoder", nil];
        }
        ...
        if (status) {
            [NSException raise:@"MacAppStore Receipt Validation Error 19" format:@"Failed to get receipt information: Decode payload", nil];
        }
...
inline static void RVNCheckReceiptIDAndVersion(NSDictionary *receiptInfo)
{
    ...
    if (![bundleID isEqualToString:kRVNBundleID]) {
        [NSException raise:@"MacAppStore Receipt Validation Error 20" format:@"Failed to check receipt ID.", nil];
    }
    
    NSString *bundleVersion = receiptInfo[kRVNReceiptInfoKeyApplicationVersion];
    if (![bundleVersion isEqualToString:kRVNBundleVersion]) {
        [NSException raise:@"MacAppStore Receipt Validation Error 21" format:@"Failed to check receipt version.", nil];
    }
}
...
inline static void RVNCheckReceiptHash(NSDictionary *receiptInfo)
{
    ...
    if (!macAddressData) {
        [NSException raise:@"MacAppStore Receipt Validation Error 22" format:@"Failed to get the primary MAC Address for checking receipt hash.", nil];
    }
    ...
    if (memcmp(digestBuffer, hashData.bytes, CC_SHA1_DIGEST_LENGTH) != 0) {
        [NSException raise:@"MacAppStore Receipt Validation Error 23" format:@"Failed to check receipt hash.", nil];
    }
}
